...
public class BuilderFederate
{
	...
	private BuilderFederateAmbassador fedamb;  // created when we connect
	...
	public void runFederate( String federateName ) throws Exception
	{
		...
		// connect
		log( "Connecting..." );
		fedamb = new BuilderFederateAmbassador( this );
		...
		while( fedamb.isRunning )
		{
			...
			if(constructionSiteAvailable - built >= 0 ) {
				ParameterHandleValueMap parameterHandleValueMap = rtiamb.getParameterHandleValueMapFactory().create(1);
				ParameterHandle addProductsCountHandle = rtiamb.getParameterHandle(startBuildingHandle, "number");
				HLAinteger32BE count = encoderFactory.createHLAinteger32BE(built);
				parameterHandleValueMap.put(addProductsCountHandle, count.toByteArray());
				rtiamb.sendInteraction(startBuildingHandle, parameterHandleValueMap, generateTag());
			}
			else
			{
				log("Building cancelled. Not enough material on the construction site");
			}
			// 9.3 request a time advance and wait until we get it
			advanceTime(builder.getTimeToNext());
...
//		// package the information into a handle set
		...
		rtiamb.subscribeObjectClassAttributes(constructionSiteHandle, attributes_plac);


		String iname = "HLAinteractionRoot.materialManagement.startBuilding";
		startBuildingHandle = rtiamb.getInteractionClassHandle( iname );
		rtiamb.publishInteractionClass(startBuildingHandle);
	...
	public static void main( String[] args )
	{
		...
		try
		{
			...
			new BuilderFederate().runFederate( federateName );
		}
